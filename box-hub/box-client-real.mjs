...
export class RealBoxClient extends BoxClient {
  ...
  async _cardFolderId(cardId) {
    ...
    for (const sub of STATUS_SUBFOLDERS) {
      ...
      const items = await this._client.folders.getFolderItems(subId);
  ...
  async findDuplicate(q) { return findDuplicate(this._client, q, await this._rootId()); }
  ...
  async uploadCard({ cardId, specMarkdown, metadata }) {
    ...
    const folderId = await ensureCardFolder(this._client, cardId, 'inbox', await this._tree());
    const fileId = await uploadText(this._client, { parentId: folderId, name: 'spec.md', content: specMarkdown });
    await applyMetadata(this._client, fileId, metadata);
    return { fileId, cardId };
  }

  // ---- Orchestrator (Person C) ----

  getMetadata(fileId) { return getCardMetadata(this._client, fileId); }
  setMetadata(fileId, patch) { return patchMetadata(this._client, fileId, patch); }
  getSpecMarkdown(fileId) { return downloadText(this._client, fileId); }

  async uploadArtifact({ cardId, name, content, area = 'card' }) {
    const tree = await this._tree();
    const parentId = area === 'logs'
      ? tree[`${ROOT}/${FOLDERS.logs}`]
      : (await this._cardFolderId(cardId)) ?? (await ensureCardFolder(this._client, cardId, 'building', tree));
    const fileId = await uploadText(this._client, { parentId, name, content });
    return { fileId };
  }
  ...
  async getArtifact({ cardId, name, area = 'card' }) {
    ...
    const items = parentId ? await this._client.folders.getFolderItems(parentId) : { entries: [] };
    const hit = (items.entries ?? []).find((e) => e.type === 'file' && e.name === name);
    if (!hit) throw new Error(`artifact not found: ${name}`);
    return downloadText(this._client, hit.id);
  }

  createTask({ fileId, message, assignee, dueDays }) {
    return createApprovalTask(this._client, { fileId, message, assignee, dueDays });
  }

  async moveCard(cardId, status) {
    const sf = statusFolder(status);
    if (!sf) return; // `failed` stays in place
    const tree = await this._tree();
    const folderId = await this._cardFolderId(cardId);
    if (!folderId) return;
    await moveFolder(this._client, folderId, tree[`${ROOT}/${FOLDERS.buildCards}/${sf}`]);
  }
